...
define([
  ...
  return declare([Evented], {
    ...
    initializeSceneMarkup: function (view) {
      ...
      this.whenLayerReady(view, "Feedback").then(layer_infos => {
            ...
            on(reset_btn, "click", () => {
              markup_layer.queryFeatures({ where: "OBJECTID > 5", outFields: ["OBJECTID"] }).then(featureSet => {
                if(featureSet.features.length) {
                  markup_layer.applyEdits({ deleteFeatures: featureSet.features }).then(applyEditsResponse => {
                    applyEditsResponse.deleteFeatureResults.forEach(deleteFeatureResult => {
                      query(`.feedback-node[data-oid="${deleteFeatureResult.objectId}"]`).orphan();
                    });
                  }, console.error);
                }
              });
            });
            domClass.remove(reset_btn, "hide");
          }
        };
        initializeReset();
        this.on("portal-user-change", initializeReset);
    ...
    addFeedbackNode: function (view, feature) {
      ...
      const feedback_node = domConstruct.create("div", {
        "data-oid": feature.attributes[feature.layer.objectIdField],
        className: "feedback-node side-nav-link"
      }, "feedback-list");
